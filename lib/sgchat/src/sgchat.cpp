...
// keep in sync with vars in gosg.go
enum NotifyType
{
  NotifyDirect = 0,
  NotifyCache = 1,
...
std::mutex SgChat::s_ConnIdMapMutex;
std::map<int, SgChat*> SgChat::s_ConnIdMap;
...
{
  m_ProfileId = GetName();
  m_SignalmeowDate = CSgGetVersion();
}
...
std::string SgChat::GetProfileId() const
{
  return m_ProfileId;
}

std::string SgChat::GetProfileDisplayName() const
{
  std::string profileDisplayName = m_Config.Get("profile_display_name");
  return profileDisplayName;
}

bool SgChat::HasFeature(ProtocolFeature p_ProtocolFeature) const
{
  // Signal allows editing within ~24 hours
  ProtocolFeature customFeatures = FeatureEditMessagesWithinTwoDays;
...
bool SgChat::SetupProfile(const std::string& p_ProfilesDir, std::string& p_ProfileId)
{
  m_IsSetup = true;

  std::cout << "\n";
  std::cout << "NOTICE:\n";
  std::cout << "This is an unofficial Signal client implementation.\n";
  ...
  int connId = CSgInit(const_cast<char*>(profileDir.c_str()), const_cast<char*>(proxyUrl.c_str()));
  if (connId == -1)
  {
    m_IsSetup = false;
    return false;
...
bool SgChat::LoadProfile(const std::string& p_ProfilesDir, const std::string& p_ProfileId)
{
  LOG_INFO("load signal profile");
  ...
  if (m_SignalmeowDate < m_ProfileDirVersion)
  {
    std::string versionWarning =
      ...
      "nchat --setup";
    LOG_WARNING("signalmeow downgrade from %d:\n%s", m_ProfileDirVersion, versionWarning.c_str());
    std::cerr << "warning: " << versionWarning << "\n";
  }
  else if (m_SignalmeowDate > m_ProfileDirVersion)
  {
    LOG_INFO("signalmeow upgrade from %d", m_ProfileDirVersion);
  }
...
bool SgChat::CloseProfile()
{
  ...
  {
    LOG_INFO("update profile to %d", m_SignalmeowDate);
    FileUtil::SetDirVersion(m_ProfileDir, m_SignalmeowDate);
  }

  int rv = CSgCleanup(m_ConnId);
...
void SgChat::InitConfig()
{
  const std::map<std::string, std::string> defaultConfig =
  {
    { "profile_display_name", "" },
  };
  const std::string configPath(m_ProfileDir + std::string("/signal.conf"));
...
bool SgChat::Login()
{
  ...
  {
    m_Running = true;
    m_Thread = std::thread(&SgChat::Process, this);

    rv = CSgLogin(m_ConnId);
...
void SgChat::SendRequest(std::shared_ptr<RequestMessage> p_RequestMessage)
{
  std::unique_lock<std::mutex> lock(m_ProcessMutex);
...
void SgChat::SetMessageHandler(const std::function<void(std::shared_ptr<ServiceMessage>)>& p_MessageHandler)
{
  m_MessageHandler = p_MessageHandler;
  m_ProcessCondVar.notify_one();
}

void SgChat::CallMessageHandler(std::shared_ptr<ServiceMessage> p_ServiceMessage)
{
...
void SgChat::PerformRequest(std::shared_ptr<RequestMessage> p_RequestMessage)
{
  ...
  switch (p_RequestMessage->GetMessageType())
  {
    ...
    case GetStatusRequestType:
      {
        // Signal doesn't support presence/online status queries
        LOG_DEBUG("get status - not supported by Signal");
      }
      break;
    ...
    case GetMessagesRequestType:
      {
        ...
        CSgGetMessages(m_ConnId,
                       const_cast<char*>(getMessagesRequest->chatId.c_str()),
                       getMessagesRequest->limit,
                       const_cast<char*>(getMessagesRequest->fromMsgId.c_str()),
                       0 /* owner */);
      }
      break;
    ...
    case MarkMessageReadRequestType:
      {
        ...
        int rv = CSgMarkMessageRead(m_ConnId, const_cast<char*>(chatId.c_str()),
    ...
    case DeleteMessageRequestType:
      {
        ...
        CSgDeleteMessage(m_ConnId, const_cast<char*>(chatId.c_str()),
                         const_cast<char*>(senderId.c_str()),
                         const_cast<char*>(msgId.c_str()));
    ...
    case DeleteChatRequestType:
      {
        ...
        CSgDeleteChat(m_ConnId, const_cast<char*>(chatId.c_str()));
        Status::Clear(m_ProfileId, Status::FlagUpdating);
    ...
    case ArchiveChatRequestType:
      {
        ...
        CSgArchiveChat(m_ConnId, const_cast<char*>(chatId.c_str()), isArchived);
        Status::Clear(m_ProfileId, Status::FlagUpdating);
      }
      break;
    ...
    case SendTypingRequestType:
      {
        ...
        int rv = CSgSendTyping(m_ConnId, const_cast<char*>(chatId.c_str()), isTyping);
    ...
    case SetStatusRequestType:
      {
        // Signal doesn't support setting online/away status
    ...
    case SendReactionRequestType:
      {
        ...
        CSgSendReaction(m_ConnId,
                        const_cast<char*>(chatId.c_str()),
                        const_cast<char*>(senderId.c_str()),
                        const_cast<char*>(msgId.c_str()),
                        const_cast<char*>(emoji.c_str()),
                        const_cast<char*>(prevEmoji.c_str())
                        );
      }
      break;
...
std::string SgChat::GetProxyUrl() const
{
  // Not supported, consider removing code.
...
void SgChat::SetProtocolUiControl(bool p_IsTakeControl)
{
  if (m_IsSetup)
  {
...
void SgChat::SetStatus(int p_Flags)
{
  Status::Set(m_ProfileId, p_Flags);
}

void SgChat::ClearStatus(int p_Flags)
{
  Status::Clear(m_ProfileId, p_Flags);
}

void SgChat::AddContactInfo(const ContactInfo& p_ContactInfo)
{
  std::unique_lock<std::mutex> lock(m_Mutex);
...
std::vector<ContactInfo> SgChat::GetContactInfos()
{
  std::unique_lock<std::mutex> lock(m_Mutex);
  return m_ContactInfos;
}

void SgChat::ClearContactInfos()
{
  std::unique_lock<std::mutex> lock(m_Mutex);
  m_ContactInfos.clear();
}

void SgChat::AddHistoryMessage(const ChatMessage& p_ChatMessage)
{
  std::unique_lock<std::mutex> lock(m_Mutex);
  m_HistoryMessages.push_back(p_ChatMessage);
}

std::vector<ChatMessage> SgChat::GetHistoryMessages()
{
  std::unique_lock<std::mutex> lock(m_Mutex);
  return m_HistoryMessages;
}

void SgChat::ClearHistoryMessages()
{
  std::unique_lock<std::mutex> lock(m_Mutex);
  m_HistoryMessages.clear();
}

void SgChat::AddInstance(int p_ConnId, SgChat* p_Instance)
{
  std::unique_lock<std::mutex> lock(s_ConnIdMapMutex);
  s_ConnIdMap[p_ConnId] = p_Instance;
}

void SgChat::RemoveInstance(int p_ConnId)
{
  std::unique_lock<std::mutex> lock(s_ConnIdMapMutex);
  s_ConnIdMap.erase(p_ConnId);
}

SgChat* SgChat::GetInstance(int p_ConnId)
{
  std::unique_lock<std::mutex> lock(s_ConnIdMapMutex);
                                ...
                                int p_TimeSent, int p_IsRead, int p_IsEdited, char* p_FromMsgId, int p_Notify)
{
  ...
  if (instance != nullptr)
  {
    ...
    if (p_Notify == NotifyCache)
    {
      ...
      instance->AddHistoryMessage(chatMessage);
    }
    else if (p_Notify == NotifySendCached)
    {
      ChatMessage chatMessage;
      ...
      newMessagesNotify = std::make_shared<NewMessagesNotify>(instance->GetProfileId());
      newMessagesNotify->success = true;
      newMessagesNotify->chatId = std::string(p_ChatId);
      newMessagesNotify->chatMessages = instance->GetHistoryMessages();
      instance->ClearHistoryMessages();
      newMessagesNotify->fromMsgId = std::string(p_FromMsgId);
      newMessagesNotify->cached = false;
      newMessagesNotify->sequence = true;
    }

    if (newMessagesNotify)
    {
      std::shared_ptr<DeferNotifyRequest> deferNotifyRequest = std::make_shared<DeferNotifyRequest>();
      deferNotifyRequest->serviceMessage = newMessagesNotify;
      instance->SendRequest(deferNotifyRequest);
...
void SgNewStatusNotify(int p_ConnId, char* p_UserId, int /*p_IsOnline*/, int /*p_TimeSeen*/)
{
  ...
  if (instance != nullptr)
  {
    // Signal doesn't support presence status, but we keep the interface for consistency
    (void)instance;
  }
...
void SgDeleteChatNotify(int p_ConnId, char* p_ChatId)
{
  ...
  if (instance != nullptr)
  {
    std::shared_ptr<DeleteChatNotify> deleteChatNotify = std::make_shared<DeleteChatNotify>(instance->GetProfileId());
    deleteChatNotify->success = true;
...
void SgReinit(int p_ConnId)
{
  ...
  if (instance != nullptr)
  {
    std::shared_ptr<ReinitRequest> reinitRequest = std::make_shared<ReinitRequest>();
...
void SgSetProtocolUiControl(int p_ConnId, int p_IsTakeControl)
{
  ...
  if (instance != nullptr)
  {
    instance->SetProtocolUiControl(p_IsTakeControl == 1);
  }
}
