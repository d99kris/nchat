...
func (wac *Conn) Send(msg interface{}) (string, error) {
	...
	case response := <-ch:
		...
		if int(resp["status"].(float64)) == 200 {
			wac.handle(ParseProtoMessage(msgProto)) // nchat customization
...
/*
BaseMessage represents a generic message.
*/
type BaseMessage struct {
	Info          MessageInfo
}

func getBaseMessage(msg *proto.WebMessageInfo) BaseMessage {
