...
void UiLite::Init()
{
  ...
  const std::string configPath(Util::GetConfigDir() + std::string("/uilite.conf"));
...
std::string UiLite::GetName()
{
  return std::string("uilite");
}
  ...
  while (m_Running)
  {
    ...
    if (FD_ISSET(STDIN_FILENO, &fds))
    {
      ...
      if (key == KEY_RESIZE)
      {
        CleanupWin();
        SetupWin();
        RequestRedraw(m_StatusWinId | m_InputWinId | m_OutputWinId);
      }
...
void UiLite::AddProtocol(Protocol* p_Protocol)
{
...
void UiLite::RemoveProtocol(Protocol* p_Protocol)
{
  std::lock_guard<std::mutex> lock(m_Lock);
...
void UiLite::UpdateChat(Chat p_Chat)
{
  std::lock_guard<std::mutex> lock(m_Lock);

  m_Chats[p_Chat.GetUniqueId()] = p_Chat;

  RequestRedraw(m_StatusWinId | m_InputWinId);
}

void UiLite::UpdateChats(std::vector<Chat> p_Chats)
{
  std::lock_guard<std::mutex> lock(m_Lock);
...
void UiLite::UpdateMessages(std::vector<Message> p_Messages, bool p_ClearChat /* = false */)
{
  std::lock_guard<std::mutex> lock(m_Lock);
...
void UiLite::RequestRedraw(char p_WinId)
{
  write(m_Sockets[1], &p_WinId, 1);
}

void UiLite::RedrawInputWin()
{
  std::lock_guard<std::mutex> lock(m_Lock);
...
void UiLite::NextPage(int p_Offset)
{
  std::lock_guard<std::mutex> lock(m_Lock);
...
void UiLite::SetCurrentChat(const std::string& p_Chat)
{
...
void UiLite::MoveInputCursor(int p_Key)
{
  std::lock_guard<std::mutex> lock(m_Lock);
...
void UiLite::NextChat(int p_Offset)
{
  std::lock_guard<std::mutex> lock(m_Lock);
...
void UiLite::InputBuf(wint_t ch)
{
  std::lock_guard<std::mutex> lock(m_Lock);
